...
function Card({ post }) {
  return (
    <div data-reveal style={s("display:flex")}>
      <Link href={`/news/${post.slug}`} className="vis-news-card" style={s("display:flex;flex-direction:column;width:100%;background:#fff;border:1px solid #ECEAE4;border-radius:16px;overflow:hidden")}>
        <div style={{ ...s(stripe), aspectRatio: "16 / 10", width: "100%", overflow: "hidden" }}>
          {post.image_url ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img className="vis-news-card-img" src={post.image_url} alt={post.title} style={s("width:100%;height:100%;object-fit:cover;display:block")} />
          ) : (
            "[ No image ]"
          )}
        </div>
        <div style={s("padding:26px 28px 30px;display:flex;flex-direction:column;flex:1")}>
          ...
          <span style={s("margin-top:18px;font-family:'Chakra Petch',sans-serif;font-weight:600;font-size:14px;letter-spacing:.04em;text-transform:uppercase;color:#8A1416")}>
            Read more <span className="vis-news-card-arrow">›</span>
          </span>
        </div>
      </Link>
    </div>
