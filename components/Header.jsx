  ...
  return (
    <header style={s("position:sticky;top:0;z-index:50;background:rgba(250,250,248,.92);backdrop-filter:saturate(140%) blur(12px);border-bottom:1px solid #ECEAE4")}>
      <div style={s("max-width:1200px;margin:0 auto;padding:12px 24px;display:flex;align-items:center;justify-content:space-between;gap:16px")}>
        ...
        {/* Mobile hamburger */}
        <button
          className={"vis-burger" + (open ? " open" : "")}
          onClick={() => setOpen((o) => !o)}
          aria-label={open ? "Close menu" : "Open menu"}
          aria-expanded={open}
          style={s("background:none;border:none;cursor:pointer;padding:8px;align-items:center;justify-content:center")}
        >
          <span className="vis-burger-box">
            <span />
            <span />
            <span />
          </span>
      ...
      <nav className={"vis-mobile-menu" + (open ? " open" : "")} style={s("background:#FAFAF8;border-bottom:1px solid #ECEAE4;padding:8px 16px 18px")}>
        {NAV.map((item) => {
