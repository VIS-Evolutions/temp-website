...
export default function SponsorForm({ action, sponsor, submitLabel = "Add sponsor" }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (file && file.size) {
        ...
        finalUrl = await uploadCoverImage(file, setStatus, "sponsor-logos");
      }
    } catch (err) {
      setStatus("");
  ...
  return (
    <form onSubmit={handleSubmit} style={s("display:flex;flex-direction:column;gap:18px")}>
      {isEdit && <input type="hidden" name="id" defaultValue={sponsor.id} />}

      <div>
        <label style={s(label)}>Sponsor name</label>
        <input name="name" defaultValue={sponsor?.name || ""} placeholder="Acme Engineering" style={s(field)} />
      </div>
      ...
      <div style={s("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,200px),1fr));gap:18px")}>
        <div>
          ...
          <input name="website" defaultValue={sponsor?.website || ""} placeholder="acme.com" style={s(field)} />
        </div>
        <div>
          <label style={s(label)}>Display order</label>
          <input name="sort_order" type="number" defaultValue={sponsor?.sort_order ?? 0} style={s(field)} />
        </div>
      </div>
      ...
      <div>
        ...
        {previewSrc && (
          ...
          <img src={previewSrc} alt="" style={s("max-width:200px;max-height:90px;object-fit:contain;background:#fff;border:1px solid #ECEAE4;border-radius:10px;padding:10px;margin-bottom:10px;display:block")} />
        )}
        <div style={s("display:flex;gap:12px;align-items:center;flex-wrap:wrap")}>
          <input ref={fileRef} name="logo" type="file" accept="image/*" onChange={onPickFile} style={s("font-family:'Public Sans',sans-serif;font-size:14px;color:#4a4843")} />
          {previewSrc && (
            <button type="button" onClick={removeLogo} style={s("background:none;border:none;cursor:pointer;font-family:'Chakra Petch',sans-serif;font-weight:600;font-size:13px;letter-spacing:.04em;text-transform:uppercase;color:#A29C92;padding:4px")}>
              Remove
            </button>
          )}
        </div>
        <p style={s("margin:8px 0 0;font-size:13px;color:#A29C92")}>PNG with transparency works best. Optimised automatically.</p>
      </div>
